...
{
    /// <summary>
    /// Represents an abstract base class for one-time execution middleware implementations that utilize three specific services.
    /// </summary>
    ...
    public abstract class OneTimeMiddleware<TService0, TService1, TService2> : Middleware<TService0, TService1, TService2>
    {
        private volatile bool hasExecuted;

        /// <summary>
        /// Initializes a new instance of the <see cref="OneTimeMiddleware{TService0, TService1, TService2}"/> class.
        /// </summary>
        ...
        protected abstract void OnInvokeService(HttpContext context, TService0 s0, TService1 s1, TService2 s2);
        ...
        protected sealed override Task OnInvokeServiceAsync(HttpContext context, TService0 s0, TService1 s1, TService2 s2)
        {
            if (!hasExecuted)
            {
                this.OnInvokeService(context, s0, s1, s2);
                hasExecuted = true;
            }
