...
{
    /// <summary>
    /// Represents an abstract base class for one-time execution middleware implementations that utilize two specific services.
    /// </summary>
    ...
    public abstract class OneTimeMiddleware<TService0, TService1> : Middleware<TService0, TService1>
    {
        private volatile bool hasExecuted;

        /// <summary>
        /// Initializes a new instance of the <see cref="OneTimeMiddleware{TService0, TService1}"/> class.
        /// </summary>
        ...
        protected abstract void OnInvokeService(HttpContext context, TService0 s0, TService1 s1);
        ...
        protected sealed override Task OnInvokeServiceAsync(HttpContext context, TService0 s0, TService1 s1)
        {
            if (!hasExecuted)
            {
                this.OnInvokeService(context, s0, s1);
                hasExecuted = true;
            }
